    ...
    private static String formatClassification(IntentClassification c) {
        ...
        if (c.isCrmAction()) {
            ...
            if ("book_appointment".equals(actionId)) {
                return "Clasificación: ACCION_CRM book_appointment. Flujo para agendar: (1) Si el usuario indica una fecha (ej. mañana, el lunes), llama getSlotsDisponibles con esa fecha en YYYY-MM-DD y ofrece las horas disponibles. (2) Cuando el usuario elija hora (ej. 09:00), pide el nombre completo: '¿Cuál es tu nombre completo?' (3) Cuando dé el nombre, pide el documento: '¿Cuál es tu número de cédula o documento?' (4) SOLO cuando tengas fecha, hora, nombre y documento dados por el usuario, llama agendarCita(servicio, fecha YYYY-MM-DD, hora HH:mm, nombreCliente, documento). NUNCA inventes nombre ni documento; si faltan, pide al usuario que los indique antes de llamar agendarCita.";
            }
